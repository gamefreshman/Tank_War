...
class LaserBeam:
    ...
    def draw(self):
        ...
        # self.end_y - self.width // 2
        color = (255, 255, 255) if self.duration % 2 == 0 else (255, 200, 200)
        pygame.draw.line(self.screen, color, 
                        (self.x - self.width / 2, self.y - self.width / 2), (self.end_x - self.width / 2, self.end_y - self.width / 2), self.width)
        
        # 绘制光晕效果
        for i in range(1, 4):
            alpha = 255 - i * 60
            glow_color = (*color[:3], alpha) if len(color) == 4 else color
...
class UIManager:
    ...
    def draw_main_menu(self):
        ...
        # 说明文字
        info_text = self.small_font.render("TAB切换射击模式 | 空格射击", True, (200, 200, 200))
